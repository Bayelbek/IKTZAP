...
function SubCategory() {
    ...
    return (
      <div className="subCategory">
          <HeaderCategory title='Подкатегории' />
          <ul className='maket_list'>
              {subCategory.filter(el => el.parent_id !== null).map((el) => (
                <li key={el.id}>
                  <CategoryMaket className="CategoryCars" image={`https://api.ikt-zap.ru/storage/images/${el.image_path}`} title={el.title} position={subCategory.map((el2) => el.parent_id == el2.id ? el2.title : null)}/>
              </li>
              ))}
          </ul>
      </div>
